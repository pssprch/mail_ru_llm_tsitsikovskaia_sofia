...
class Tokenizer:
    ...
    def build_vocab(self, corpus: List[str]) -> None:
        assert len(corpus)
        all_tokens = set()
        for text in tqdm(corpus, desc="build_vocab"):
...
class StatLM:  # (ModelTemplate):
    ...
    def generate_response(self, text: str, generation_config: GenerationConfig):
        text = "Собеседник: " + text + " Ты: "
        text = remove_punct(text)

        all_tokens = self.tokenizer.encode(text, append_eos_token=False)
        print(f"Text: `{text}`")
        generation_config.max_tokens = min(generation_config.max_tokens, len(all_tokens) + 40)
        result = self.generate_text(text, generation_config=generation_config)
        all_tokens_generated = result["all_tokens"][len(all_tokens):]
        new_text = self.tokenizer.decode(all_tokens_generated, generation_config.remove_special_tokens)
        print("All generated:", result["total_text"])
        new_text = new_text.split("собеседник :")[0]
        new_text = new_text.split("ты :")[0]
        result["generated_text"] = new_text
        print("Text after postprocessing:", new_text)
        return result

    def generate(self, text: str, generation_config: Dict) -> str:
        return self.generate_response(text, generation_config)["generated_text"]
